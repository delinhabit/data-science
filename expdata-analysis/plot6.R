...
mv_emissions_by_year <- nei_data %>%
    ...
    mutate(location = ifelse(fips == "24510", "Baltimore, MA", "Los Angeles, CA")) %>%
    group_by(year, location) %>%
    summarise(total = sum(Emissions)) %>%
    mutate(total = log(total))

# Allow more space for the legend on the right
png("plot6.png")

ggplot(mv_emissions_by_year, aes(year, total, col = location)) +
    geom_point() +
    geom_smooth(method = "lm") +
    scale_size_area() +
    labs(x = "Year", y = expression("log " * PM[2.5]), col = "Location") +
    ggtitle("Comparative evolution of motor vehicle emissions")
